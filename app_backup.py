...
from dkt.models.utils import load_state_dict
...
MODEL_TYPE = "1.3B"
# LOAD_MODEL_PATH = "DKT_models/T2SQNet_glassverse_cleargrasp_HISS_DREDS_DREDS_glassverse_1.3B_depth_70K_lora.safetensors"
LOAD_MODEL_PATH='DKT_models/T2SQNet_glassverse_cleargrasp_HISS_DREDS_DREDS_glassverse_interiorverse-1.3B-rgb_depth.safetensors'
...
LOAD_MODEL_PATH_14B = "DKT_models/T2SQNet_glassverse_cleargrasp_HISS_DREDS_DREDS_14B_depth_70k_lora.safetensors"
LOAD_MODEL_PATH_MOGE = "checkpoints/moge_ckpt/moge-2-vitl-normal/model.pt"
...
def load_moge_model(device="cuda:0"):
    ...
    MOGE_MODULE = MoGeModel.from_pretrained(LOAD_MODEL_PATH_MOGE).to(device)
    return MOGE_MODULE
...
def load_model_1_3b(device="cuda:0"):
    ...
    if LORA:
        ...
        pipe.load_lora(pipe.dit, LOAD_MODEL_PATH, alpha=new_weight_alpha)
        logger.info(f"Loaded LoRA model from {LOAD_MODEL_PATH}, with alpha {new_weight_alpha}")
    else:
        pipe.dit.load_state_dict(load_state_dict(LOAD_MODEL_PATH))
        logger.info(f"Loaded full model from {LOAD_MODEL_PATH}")
...
def load_model_14b(device="cuda:1"):
    global PIPE_14B
    
    if PIPE_14B is not None:
        return PIPE_14B
    ...
    if LORA:
        new_weight_alpha = 1.0
        pipe.load_lora(pipe.dit, LOAD_MODEL_PATH_14B, alpha=new_weight_alpha)
        logger.info(f"Loaded LoRA model from {LOAD_MODEL_PATH_14B}, with alpha {new_weight_alpha}")
    else:
        pipe.dit.load_state_dict(load_state_dict(LOAD_MODEL_PATH_14B))
        logger.info(f"Loaded full model from {LOAD_MODEL_PATH_14B}")
...
def get_model(model_size):
    if model_size == "1.3B":
        ...
        assert PIPE_14B is not None, "14B model not initialized"
        return PIPE_14B
    else:
...
def process_video(
    ...
    try:
        ...
        pc_save_dir = join(cur_save_dir, 'pointclouds')
        ...
        for idx in tqdm(indices):
            ...
            glb_filename = f'{pc_save_dir}/{timestamp}_{idx:02d}.glb'
...
if __name__ == '__main__':
    ...
    #* the 14B  and 1.3B models can not be loaded on the same L20 GPU, so we need to load them on different GPUs
    ...
    load_model_1_3b(device=device_1_3b)
    load_model_14b(device=device_14b)
    load_moge_model(device=device_moge)
